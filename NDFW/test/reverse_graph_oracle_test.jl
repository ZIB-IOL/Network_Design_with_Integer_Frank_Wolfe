...
blmo = NDFW.initialize_lmo("BNDLMO-P", optimizer, g1.graph, gparams, use_bigm, use_adaptive, use_reverse)
...
xi = Boscia.compute_extreme_point(lmo, gb)

println("IFW Objective on IFW: $(fifw(xi))")
println("IFW Objective on BLMO: $(fifw(xb))")
println("BNDLMO-P Objective on IFW: $(fblmo(xi))")
println("BNDLMO-P Objective on BLMO: $(fblmo(xb))")
