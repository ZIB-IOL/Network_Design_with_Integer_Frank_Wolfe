...
"""
function direct_scip_solve(econfig, cost_of_expansion, sres, edges2remove=0)
    ...
    x = [MOI.VariableIndex(i) for i = 1:var_cnt]
    x_agg = [MOI.VariableIndex(i) for i = (var_cnt+1):(var_cnt+params.no_edges)]
    x_nd = [MOI.VariableIndex(i) for i = (var_cnt+params.no_edges+1):(var_cnt+params.no_edges+new_edge_count)]
    tb = MOI.add_variables(optimizer, params.no_edges)
    ...
    for i in eachindex(x_agg)
        ...
        term = :($(const_term) 
                + $(om) * (x[$(x_agg[i])] + $(ia) * (x[$(x_agg[i])]^($powt)) / $(denom))
                    - x[$(tb[i])] <= 0.0
                )
