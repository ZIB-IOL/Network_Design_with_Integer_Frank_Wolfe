...
function BPR(x::Vector{Float64}, td::NDFW.TA_Data)
    bpr = similar(x)
...
function objective(x::Vector{Float64}, td::NDFW.TA_Data)
...
function hessian(x::Vector{Float64}, td::NDFW.TA_Data)
...
function hessian_diag(x::Vector{Float64}, td::NDFW.TA_Data)
...
function all_or_nothing_single(travel_time::Vector{Float64}, td::NDFW.TA_Data, graph, link_dic)
    local state::Graphs.DijkstraState{Float64, Int}
    x = zeros(size(td.init_node))
    no_nodes = Graphs.nv(graph)

    distmx = zeros(no_nodes, no_nodes)

    for r in 1:size(td.travel_demand)[1]
        # for each origin node r, find shortest paths to all destination nodes
        state = TA_dijkstra_shortest_paths_default(graph, travel_time, r, td.init_node, td.term_node, td.first_thru_node, distmx)
...
function all_or_nothing(travel_time::Vector{Float64}, td::NDFW.TA_Data, graph, link_dic)
...
function ta_frank_wolfe(td::NDFW.TA_Data; method=:bfw, max_iter_no=2000, step=:exact, log=:off, tol=1e-3)
